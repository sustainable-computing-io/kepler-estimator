from common import load_model_by_pickle, load_model_by_keras, transform_and_predict, load_model_by_json
    ...
    def __init__(self, model_name, output_type, model_file, features, fe_files, component_init=False):
        ...
        if self.comp_type:
            ...
            for comp, model_metadata in model_info.items():
                model = KerasModel(self.name, self.output_type.name, model_metadata['model_file'], model_metadata['features'], model_metadata['fe_files'], component_init=True)
                self.models[comp] = model
        else:
            self.model = load_model_by_keras(output_type, model_file)
            self.fe_list = []
